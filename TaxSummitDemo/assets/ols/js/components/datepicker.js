...
define(['jquery'], function ($) {
    ...
    var DatePicker = function (elem, options) {
        ...
        this.metadata = this.$input.data('datepicker-options');
    };
    ...
    DatePicker.prototype.init = function () {
        ...
        //Prevent a datepicker from being initialized more than once.
        if($("#"+ID_PREFIXES.calIcon + this.$input.attr('id')).length> 0){
            return;
        }
        ...
        this.$input
            .on('blur', _events._inputBlur)
            .on('keyup', _events._inputKeyup);

        // Create icon
        $icon = $('<button/>', {
                    'type': 'button',
                    'class': 'cui-icon cui-c-datepicker',
                    'title': ICON_TOOLTIP.show,
                    'tabindex': '1',
                    'id': ID_PREFIXES.calIcon + this.$input.attr('id'),
                })
                .text('Date picker')
                .on('click', _events._iconClick);

        // Insert the icon:

        // Look for a field wrapper
        $fieldMain = this.$input.closest('.cui-field-main');
        $fieldCollection = this.$input.closest('.cui-field-collection');

        // We're not in a field, so just stick the icon right after the input
        if (!$fieldMain.length) {
            $icon.insertAfter(this.$input);
        }
        else if($fieldCollection.length && $fieldCollection.length > 0){
            $icon.insertAfter(this.$input);
        }
        // In a field
        else {
            // Ensure there's a `div.cui-controls`
            $controls = $fieldMain.find('.cui-controls');
    ...
    //FIXME: the `_imgPath` global is never actually used by anything. This function can probably go away, too. (CP 7/10/2017)
    ...
    var _customize = function _customize (settings) {
            ...
            var i;
            var j = 0;
            var k = _mySettings.datePickers.length;
            ...
            // If only one object was sent, convert it to an array
                ...
                if (dp && dp.inputId) {
                    if ($.trim(dp.inputId).length > 0) {
                        ...
                        _mySettings.datePickers[k] = {
                            ...
                            useFastQuarterButton: dp.useFastQuarterButton || _defaultSettings.datePickers[0].useFastQuarterButton,
                        };
    ...
    _priv.getDatePickerHtml = function _getDatePickerHtml (dmyCal, inputId) {
        ...
        while (i < lastDayMonth) {
            ...
            var dateParsed = _priv.processDate($.trim(document.getElementById(inputId).value), inputId);
    ...
    _priv.getHeaderHtml = function _getHeaderHtml (dmyCal, inputId) {
        ...
        var fastButtonType = {className: 'Year', text: 'year'};

        if (settings.useFastQuarterButton) {
            fastButtonType = {className: 'Qtr', text: 'quarter'};
        }
        ...
        if (dmyCal.month === (minDate.getMonth() + 1) && dmyCal.year === minDate.getFullYear()) {
            ...
            html += '<ul class="lIcons"><li><a href="#" class="fastNavPrev' + fastButtonType.className + '" title="Previous ' + fastButtonType.text + '" tabindex="1"></a></li><li><a href="#" class="navPrevMon" title="Previous month" tabindex="1"></a></li></ul>';
        }
        ...
        if (dmyCal.month === (maxDate.getMonth() + 1) && dmyCal.year === maxDate.getFullYear()) {
            ...
            html += '<ul class="rIcons"><li><a href="#" class="navNextMon" title="Next month" tabindex="1"></a></li><li><a href="#" class="fastNavNext' + fastButtonType.className + '" title="Next ' + fastButtonType.text + '" tabindex="1"></a></li></ul>';
        }
    ...
    _priv.getMonthYearFromCalHeader = function _getMonthYearFromCalHeader (cal) {
        ...
        var yr = parseInt(splitMonYr[1], 10);
